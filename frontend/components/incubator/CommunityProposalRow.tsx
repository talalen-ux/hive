...
export function CommunityProposalRow({
  proposal,
}: {
  ...
  const myChoice = state.myVotes[`community:${proposal.id}`];

  async function cast(choice: "yes" | "no" | "abstain") {
    setBusy(choice);
    try {
      actions.voteOnCommunityProposal(proposal.id, choice);
    } finally {
      setBusy(null);
    }
  }

  const total = communityTotal(proposal);
    ...
    >
      <div className="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-4">
        <div className="min-w-0">
          <div className="flex items-center gap-2 text-[10px] uppercase tracking-wider2 text-honey-soft/50">
            <span>community</span>
            <span>·</span>
            <span className="numeric">{fmtAddress(proposal.submittedBy)}</span>
            {proposal.status === "PASSED" && (
              <span className="ml-1 rounded-full bg-honey/20 px-2 py-0.5 text-[9px] tracking-wider2 text-honey-soft">
                passed → task
              </span>
            )}
            {proposal.status === "REJECTED" && (
              <span className="ml-1 rounded-full bg-honey-soft/[0.06] px-2 py-0.5 text-[9px] tracking-wider2 text-honey-soft/55">
                rejected
              </span>
            )}
          </div>
          ...
          <p className="mt-1 text-[13px] text-honey-soft/65 leading-relaxed">
        ...
        <div className="text-right shrink-0">
          ...
          <div className="mt-1 text-honey-soft numeric">
            {closed ? "—" : <Countdown end={proposal.votingEnd} />}
          </div>
        </div>
      </div>
      ...
      <div className="mt-5">
        ...
        <div className="mt-2 flex items-center justify-between text-[11px] text-honey-soft/55 numeric">
          <span>
            <span className="text-honey-soft">Yes</span> {fmtNum(proposal.yes)} (
            {(yesPct * 100).toFixed(0)}%)
          </span>
          <span>
            <span className="text-honey-soft/65">No</span> {fmtNum(proposal.no)} (
            {(noPct * 100).toFixed(0)}%)
          </span>
        </div>
      </div>
      ...
      <div className="mt-4">
        <div className="flex items-center justify-between text-[10px] uppercase tracking-wider2 text-honey-soft/50">
          <span>Quorum {communityQuorumMet(proposal) ? "met" : "pending"}</span>
      ...
      {!closed ? (
        ...
        <div className="mt-5 text-center text-[11px] text-honey-soft/55">
          {proposal.status === "PASSED"
            ? "Voting closed · proposal accepted and queued as a task on this project."
            : "Voting closed · did not reach the bar."}
        </div>
      )}
    </motion.div>
  );
